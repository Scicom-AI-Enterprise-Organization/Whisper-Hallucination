...
to suppress, this time genuinely spoken and labelled with its transcript. In twelve matched
pairs of fine-tunes that differ by nothing else, those clips lower word emission on real
voice-free audio in 10 pairs and raise phrase recovery in 11, and among the ten pairs that hold
English accuracy it is 10 out of 10. The best checkpoint takes silence hallucination from
...
Two things we would not have known without measuring both halves on every run. Training loss
ranks these models backwards, because a blank-heavy mix is cheap to fit and the mix that trains
to the better loss is the worse model. And every fine-tune we produced is worse than the base
model on FLEURS while LibriSpeech stays flat at 0.034 to 0.037, so an English-only accuracy
guard would have reported no cost at all. The benchmark, the lexicon, the synthetic corpus and
