...
So the defect is not confined to one model that has been superseded. It is in the model most
production systems use, and it is in the teacher that Distil-Whisper, Kotoba-Whisper and
Granary distilled from. Section~\ref{sec:distil} argues that the way those corpora are built
...
\texttt{terima kasih kerana menonton} appear over and over on audio containing none of them. We merged four public sources into 40,891
phrases across 100 languages.
...
throughout, so every configuration is a matched pair.
...
\textbf{Learning to stop lives in the feed-forward layers.} On the \texttt{all} mix, across all
fifteen matched rank and learning-rate cells, adapting \texttt{fc1} and \texttt{fc2} as well as
the attention projections lowers word emission on wild audio in \textbf{15 of 15}, by a median
of 0.133. Attention-only adapters span 0.762 to 0.911 across the whole rank range, against a
