...
\begin{abstract}
Whisper changed what open speech recognition means: one checkpoint, 99 languages, no
per-language tuning, running on a laptop. It is still the default four years on, and it is
still the teacher that Distil-Whisper, Kotoba-Whisper and NVIDIA's Granary are all distilled
from. It also writes sentences nobody said. On 42 clips of pure room tone, \texttt{whisper-large-v3}
...
transcription used to mean a model per language and a pipeline per domain. Whisper made it one
MIT-licensed checkpoint covering 99 languages, with no per-language tuning, running on a
laptop. Four years later it is still the default, and it is good: we measured it on 20 FLEURS
...
It is no longer the most accurate model, and that makes the point stronger rather than weaker.
Canary-1B-v2 reports an average word error rate of 8.1\% against \texttt{whisper-large-v3}'s
9.9\% on its 25 European languages, SeamlessM4T \citep{seamless2023m4t} reported a 45\%
relative reduction over \texttt{whisper-large-v2} on FLEURS across 77 overlapping languages,
and Omnilingual ASR \citep{keren2025omnilingual} covers more than 1,600 languages. The first
two of those were trained on Whisper output. Fixing the teacher is worth doing precisely
because the students have already overtaken it, and Section~\ref{sec:distil} argues that the
way their corpora are built lets its worst behaviour through while deleting the evidence for
it.
